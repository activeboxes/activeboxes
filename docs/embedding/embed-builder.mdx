...
This documentation explains how to embed the ActiveBoxes iframe inside your application and customize it.
...
```html
<script src="https://cdn.activeboxes.org/sdk/embed/0.3.4.js">
</script>
<script>
...
| --- | --- | --- | --- |
| prefix | ❌ | string | Some customers have an embedding prefix, like this `<embedding_url_prefix>/<Activepieces_url>`. For example if the prefix is `/automation` and the ActiveBoxes url is `/flows` the full url would be `/automation/flows`. |
| instanceUrl| ✅ |  string | The url of the instance hosting ActiveBoxes, could be https://cloud.activeboxes.org if you are a cloud user. |
| jwtToken | ✅  |  string | The jwt token you generated to authenticate your users to ActiveBoxes. |
| embedding.containerId | ❌ | string | The html element's id that is going to be containing ActiveBoxes's iframe. |
...
| navigation.handler  | ❌ | `({route:string}) => void`  | If defined the callback will be triggered each time a route in ActiveBoxes changes, you can read more about it [here](/embedding/navigation)
<Tip>
