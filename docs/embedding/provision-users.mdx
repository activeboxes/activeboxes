...
description: "Automatically authenticate your SaaS users to your ActiveBoxes instance"
icon: 'user'
---
...
In ActiveBoxes, there are **Projects** and **Users**. Each project is provisioned with their corresponding workspace, project, or team in your SaaS. The users are then mapped to the respective users in ActiveBoxes.
...
<Steps>
  <Step title="Step 1: Obtain Signing Key">
    ...
    This will generate a public and private key pair. The public key will be used by ActiveBoxes to verify the signature of the JWT tokens you send. The private key will be used by you to sign the JWT tokens.

    <Warning>
      Please store your private key in a safe place, as it will not be stored in ActiveBoxes.
    </Warning>
  </Step>

  <Step title="Step 2: Generate a JWT">
    The signing key will be used to generate JWT tokens for the currently logged-in user on your website, which will then be sent to the ActiveBoxes Iframe as a query parameter to authenticate the user and exchange the token for a longer lived token.
    ...
    | role              | Role of the user in the ActiveBoxes project (e.g., **EDITOR**, **VIEWER**, **ADMIN**)           |
