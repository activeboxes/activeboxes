...
By default, navigating within your embedded instance of ActiveBoxes doesn't affect the client's browser history or viewed URL. ActiveBoxes only provide a **handler**, that trigger on every route change in the **iframe**.

## Automatically Sync URL 

You can use the following snippet when configuring the SDK, which will implement a handler that syncs the ActiveBoxes iframe with your browser:
