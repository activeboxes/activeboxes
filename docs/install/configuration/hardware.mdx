...
icon: "server"
description: "Specifications for hosting ActiveBoxes"
---
...
ActiveBoxes is designed to be memory-intensive rather than CPU-intensive. A modest instance will suffice for most scenarios, but requirements can vary based on specific use cases.

| Component           | Memory (RAM) | CPU Cores | Notes                                                                 |
|---------------------|--------------|-----------|-----------------------------------------------------------------------|
| PostgreSQL          | 1 GB         | 1         |                                                                       |
| Redis               | 1 GB         | 1         |                                                                       |
| ActiveBoxes        | 8 GB         | 2         | For high availability, consider deploying across multiple machines. Set `FLOW_WORKER_CONCURRENCY` to `25` for optimal performance. |
...
Redis requires minimal scaling as it primarily stores jobs during processing. ActiveBoxes leverages BullMQ, capable of handling a substantial number of jobs per second.
...
<Tip>
**Scaling Tip:** The ActiveBoxes container is stateless, allowing for seamless horizontal scaling.
</Tip>
...
ActiveBoxes ensures no request is lost; all requests are queued. In the event of a spike, requests will be processed later, which is acceptable as most flows are asynchronous, with synchronous flows being prioritized.
