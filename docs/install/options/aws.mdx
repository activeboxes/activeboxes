---
title: "AWS (Pulumi)"
description: "Get ActiveBoxes up & running on AWS with Pulumi for IaC"
---
...
If you're already familiar with Pulumi Cloud and have [integrated their services with your AWS account](https://www.pulumi.com/docs/pulumi-cloud/deployments/oidc/aws/#configuring-openid-connect-for-aws), you can use the button below to deploy ActiveBoxes in a few clicks. 
The template will deploy the latest ActiveBoxes image that's available on [Docker Hub](https://hub.docker.com/r/activeboxes/activeboxes).

[![Deploy with Pulumi](https://get.pulumi.com/new/button.svg)](https://app.pulumi.com/new?template=https://github.com/activeboxes/activeboxes/tree/main/deploy/pulumi)

## Deploy from a local environment

Or, if you're currently using an S3 bucket to maintain your Pulumi state, you can scaffold and deploy ActiveBoxes direct from Docker Hub using the template below in just few commands:

```bash
$ mkdir deploy-activepieces && cd deploy-activepieces 
$ pulumi new https://github.com/activeboxes/activeboxes/tree/main/deploy/pulumi
$ pulumi up
```
...
Opting out of this will mean relying on using the application load balancer's url over standard HTTP to access your ActiveBoxes deployment.

For a full list of all the currently available configuration options, take a look at the [ActiveBoxes Pulumi template file on GitHub](https://github.com/activeboxes/activeboxes/tree/main/deploy/pulumi/Pulumi.yaml).

## Setting up Pulumi for the first time

If you're new to Pulumi then read on to get your local dev environment setup to be able to deploy ActiveBoxes.
...
4. Create an S3 bucket which we'll use to maintain the state of all the various service we'll provision for our ActiveBoxes deployment:
...
```
6. Next we're going to use the ActiveBoxes Pulumi deploy template to create a new project, a stack in that project and then kick off the deploy:
...
The config value of interest here is the `activePiecesUrl` as that is the URL for our ActiveBoxes deployment. 
...
Congratulations! You have successfully deployed ActiveBoxes to AWS.

## Deploy a locally built ActiveBoxes Docker image

To deploy a locally built image instead of using the official Docker Hub image, read on.

1. Clone the ActiveBoxes repo locally:

```bash
git clone https://github.com/activeboxes/activeboxes
```
...
Congratulations! You have successfully deployed ActiveBoxes into AWS using a locally built Docker image.
...
Or maybe you'd still like to deploy the official ActiveBoxes Docker image instead of a local build, but would like to change some of the services. Simply set the `deployLocalBuild` config option in the stack file to `false` and make whatever changes you'd like to the `index.ts` file.   
