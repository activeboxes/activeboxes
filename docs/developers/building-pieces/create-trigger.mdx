...
> = {
  ...
  items: async ({ auth, propsValue, lastFetchEpochMS }) => {
    const request: HttpRequest = {
      ...
      url: 'https://cloud.activeboxes.org/api/v1/webhooks/aHlEaNLc6vcF1nY2XJ2ed/sync',
      headers: {
        authorization: auth,
      },
    };
...
To make the trigger readable by ActiveBoxes, add it to the array of triggers in the piece definition.
...
To test the trigger, use the load sample data from flow builder in ActiveBoxes. It should function as shown in the screenshot.
